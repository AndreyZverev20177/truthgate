...
def _canonical_class(raw: str) -> VulnerabilityClass | None:
    """Кэш-обёртка — единая точка нормализации строки в enum."""
    return normalize_vuln_class(raw)


def dedup_key(c: Candidate) -> str:
    """Стабильный ключ для дедупликации кандидатов.

    Использует канонический enum-value (не сырой `bug_class`), чтобы `"SQLi"`,
    `"sqli"`, `"sql_injection"` давали ОДИН ключ.
    """
...
def validate_candidate(c: Candidate, ctx: ValidatorContext) -> Verdict:
    """Прогнать кандидата через зарегистрированный валидатор.

    Не бросает валидатор-исключения наружу — оборачивает в `Verdict(is_real=False)`.
    Единственная реальная ошибка — `NoValidatorError` (класс не мапится или
    валидатор не зарегистрирован); её решает уровень выше.
    """
    vc = _canonical_class(c.bug_class)
    if vc is None:
