...
class TestDirectEnumValue:
    def test_lower_snake_case_direct_hit(self) -> None:
        """Строка совпадает с enum.value → возвращает enum, без таблицы алиасов."""
...
class TestAliases:
    def test_bola_maps_to_idor(self) -> None:
        """BOLA (OWASP API-1) → IDOR — исторический синоним."""
    ...
    def test_path_traversal_maps_to_lfi(self) -> None:
        """Классические синонимы directory/path traversal."""
    ...
    def test_prompt_inject_llm_variants(self) -> None:
        """LLM-агенты присылают разные слэнговые варианты."""
...
class TestUnknown:
    def test_returns_none_on_unknown(self) -> None:
        ...
        assert normalize_vuln_class("csrf") is None  # CSRF не в списке M1-M2
...
class TestAliasesFor:
    def test_returns_sorted_aliases(self) -> None:
        """Для IDOR должно быть минимум bola + два broken_object_..."""
    ...
    def test_no_alias_for_direct_enum(self) -> None:
        """У класса без строковых синонимов aliases_for пуст (напр. XSS не в таблице)."""
        # XSS есть в таблице как cross_site_scripting, значит не пуст;
        # проверим на bfla — там broken_function_level_authz есть.
