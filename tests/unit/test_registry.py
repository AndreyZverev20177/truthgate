"""Unit-тесты нового enum-based реестра валидаторов (`tools.validators.registry`)."""
...
class _Dummy(BaseValidator):
    """Минимальный валидатор для проверок регистрации."""
...
class _MismatchedClass(BaseValidator):
    """Валидатор, у которого vulnerability_class не совпадает с ключом register()."""
...
class TestRegistry:
    def test_register_and_get_by_enum(self) -> None:
        """Ключ — только `VulnerabilityClass`, никаких строк."""
    ...
    def test_get_missing_returns_none(self) -> None:
        """Класс без валидатора → None (не исключение)."""
    ...
    def test_register_rejects_non_base_validator(self) -> None:
        """register() принимает только наследников BaseValidator."""
    ...
    def test_register_rejects_mismatched_class(self) -> None:
        """register(SQLI) на классе, у которого vulnerability_class = SSRF → ошибка."""
