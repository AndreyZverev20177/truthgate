...
def register(
    ...
    Инстанциирует класс без аргументов (валидаторы должны иметь `__init__()`
    без обязательных параметров — конфиг живёт в `ValidatorContext`) и кладёт
    в реестр. Повторная регистрация того же класса перезаписывает предыдущую.
    """
